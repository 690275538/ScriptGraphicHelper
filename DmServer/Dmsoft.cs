...
public class Dmsoft
{
    ...
    public byte[] GetRectData(int sx, int sy, int ex, int ey)
    {
        int width = ex - sx;
        int height = ey - sy;
        ...
        for (int i = sy; i < ey; i++)
        {
            int location = sx * 4 + (ImageWidth * 4) * i;
            for (int j = sx; j < ex; j++)
            {
                data[site] = ImageData[location];
                data[site + 1] = ImageData[location + 1];
                data[site + 2] = ImageData[location + 2];
                data[site + 3] = ImageData[location + 3];
                location += 4;
                site += 4;
            }
        }
    ...
    {
        byte[] oldByte = ImageData;
        try
        {
            IntPtr imageData = (IntPtr)GetScreenData(0, 0, ImageWidth, ImageHeight);
            ImageData = new byte[ImageWidth * 4 * ImageHeight];
            Marshal.Copy(imageData, ImageData, 0, ImageWidth * 4 * ImageHeight);
            return true;
        }
        catch
        {
    ...
    public byte[] GetPixel(int x, int y)
    {
        ...
        try
        {
            ...
            {
                int location = x * 4 + ImageWidth * 4 * y;
                result[0] = ImageData[location + 2];
                result[1] = ImageData[location + 1];
                result[2] = ImageData[location];
            }
        }
        catch
        {
    ...
    public bool CompareColor(byte[] findColor, byte[] OffsetColor, double similarity = 95, byte offset = 1, int findX = 0, int findY = 0)
    {
        int x;
        int y;
        int offsetX = 0;
        int offsetY = 0;
        int size = (offset + offset + 1) * (offset + offset + 1);
        for (int i = 0; i < size; i++)
        {
            ...
            if (x >= 0 && x < ImageWidth && y >= 0 && y < ImageHeight)
            {
                ...
                if (Math.Abs(pixel[0] - findColor[0]) <= (OffsetColor[0] + similarity) &&
                    Math.Abs(pixel[1] - findColor[1]) <= (OffsetColor[1] + similarity) &&
                    Math.Abs(pixel[2] - findColor[2]) <= (OffsetColor[2] + similarity))
                {
                    return true;
                }
            }
    ...
    public bool CompareColorEx(string descriptions, double similarity = 95, byte offset = 1, int X = 0, int Y = 0)
    {
        int findX;
        int findY;
        ...
        if (description.Length != 0)
        {
            ...
            {
                string[] _description = description[i].Split('|');
                string[] colors = _description[2].Split('-');
                byte[] findColor = { 0, 0, 0 };
                findColor[0] = Convert.ToByte(colors[0].Substring(0, 2), 16);
                findColor[1] = Convert.ToByte(colors[0].Substring(2, 2), 16);
                findColor[2] = Convert.ToByte(colors[0].Substring(4, 2), 16);
                findX = X + int.Parse(_description[0]);
                findY = Y + int.Parse(_description[1]);
                if (findX < 0 || findY < 0 || findX > ImageWidth || findY > ImageHeight)
                {
                    return false;
                }
                byte[] OffsetColor = { 0, 0, 0 };
                if (colors.Length > 1)
                {
                    OffsetColor[0] = Convert.ToByte(colors[1].Substring(0, 2), 16);
                    OffsetColor[1] = Convert.ToByte(colors[1].Substring(2, 2), 16);
                    OffsetColor[2] = Convert.ToByte(colors[1].Substring(4, 2), 16);
                }
                if (!CompareColor(findColor, OffsetColor, similarity, offset, findX, findY))
                {
                    return false;
                }
            }
    ...
    public Point FindMultiColor(int startX, int startY, int endX, int endY, string[] description, byte sim = 95, byte offset = 0)
    {
        return FindMultiColor(startX, startY, endX, endY, description[0], description[1], sim, offset);
    }
    public Point FindMultiColor(int startX, int startY, int endX, int endY, string firstDescription, string offsetDescription, byte sim = 95, byte offset = 1)
    {
        if (startX < endX && endX <= ImageWidth && startY < endY && endY <= ImageHeight)
        {
            string[] description = firstDescription.Split('-');
            byte findR = Convert.ToByte(description[0].Substring(0, 2), 16);
            byte findG = Convert.ToByte(description[0].Substring(2, 2), 16);
            byte findB = Convert.ToByte(description[0].Substring(4, 2), 16);
            byte offsetR = 0;
            byte offsetG = 0;
            byte offsetB = 0;
            if (description.Length > 1)
            {
                offsetR = Convert.ToByte(description[1].Substring(0, 2), 16);
                offsetG = Convert.ToByte(description[1].Substring(2, 2), 16);
                offsetB = Convert.ToByte(description[1].Substring(4, 2), 16);
            }
            double similarity = 255 - 255 * (sim / 100.0);
            for (int i = startY; i <= endY; i++)
            {
                int location = startX * 4 + i * ImageWidth * 4;
                for (int j = startX; j <= endX; j++)
                {
                    if (Math.Abs(ImageData[location + 2] - findR) <= offsetR + similarity)
                    {
                        if (Math.Abs(ImageData[location + 1] - findG) <= offsetG + similarity)
                        {
                            if (Math.Abs(ImageData[location] - findB) <= offsetB + similarity)
                            {
                                if (CompareColorEx(offsetDescription, similarity, offset, j, i))
                                {
    ...
    public Point FindPic(int sx, int sy, int ex, int ey, string path, int sim = 90)
    {
        int x = -1;
        int y = -1;
        string _path = Directory.GetCurrentDirectory() + "\\image\\" + path;
        if (FindPic(sx, sy, ex, ey, _path, "121212", (double)sim / 100, 0, out x, out y) == -1)
        {
            return new Point(-1, -1);
        }
        return new Point(x, y);
    }
    public void LeftClick(int X, int Y)//移动点击
    {
        MoveTo(X, Y);
        Thread.Sleep(50);
        LeftClick();
    }
    public void LeftClick(Point point)//移动点击
    {
        MoveTo((int)point.X, (int)point.Y);
        Thread.Sleep(50);
        LeftClick();
    }
